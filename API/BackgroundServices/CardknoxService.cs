...
{
    public class CardknoxService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    // FOR TESTING: Run after 1 minutes instead of waiting for next month
                    //var delay = TimeSpan.FromMinutes(1);
                    //var nextRun = DateTime.Now.Add(delay);

                    // FOR PRODUCTION: Uncomment below and comment out the above 2 lines
                    var now = DateTime.Now;
                    var nextRun = new DateTime(now.Year, now.Month, 1)
                                      .AddMonths(1)
                                      .AddSeconds(1); // 1st of next month at 00:00:01
                    var delay = nextRun - now;
                    _logger.Information("Next monthly job scheduled at {time}", nextRun);

                    await Task.Delay(delay, stoppingToken);

                    if (!stoppingToken.IsCancellationRequested)
                    {
                        await RunMonthlyTask(stoppingToken);
                    }
                }
                catch (OperationCanceledException)
                {
                    _logger.Information("Cardknox Service is shutting down");
                    break;
                }
                catch (Exception ex)
                {
                    _logger.Error(ex, "Error in monthly task execution loop");
                    await Task.Delay(TimeSpan.FromMinutes(5), stoppingToken);
                }
            }

            _logger.Information("Cardknox Service Stopped at {time}", DateTime.Now);
        }
                    ...
                    try
                    {
                        ...
                        if (cards == null || !cards.Any())
                        {
                            await _paymentHistoryService.SaveFailure(
                                ...
                                false,
                                1,
                                null, null, false, charge.ChargeId, "Declined");
                        ...
                        foreach (var card in cards)
                        {
                            ...
                            else
                            {
                                await _paymentHistoryService.SaveFailure(
                                    ...
                                    "Monthly Charges",
                                    false,
                                    1, card.PaymentMethodId, null, false, charge.ChargeId, result.Status);
                            }
                        }
                        ...
                        foreach (var card in cards)
                        {
                            ...
                            else
                            {
                                await _paymentHistoryService.SaveFailure(
                                    ...
                                    "Monthly Charges",
                                    false,
                                    item.AttemptCount, card.PaymentMethodId, null, false, item.ChargeId, result.Status);                                
                            }
                        }
                        if (!isSuccess)
                        {
